...
**Exact result.** Any two distinct positive full transform matrices identify every complete three-state triangle, with all six rates initially unknown, uniquely among the reciprocal models with at most three states. The inverse is rational and locally Lipschitz at each such source. Three distinct positive matrices remain sufficient throughout the original class, and three are necessary in the worst case when an observed-endpoint rate is uncalibrated: at a hidden leaf, every pair of distinct positive arguments leaves a positive one-parameter generator ambiguity. One full matrix is insufficient in general even on complete triangles.
