...
The earlier unresolved existence question has now been answered: a minimal five-state kernel under a five-state cap and unknown bidirected topology can have bounded but nonunique entropy. The [balanced quartic endpoint](theta-balanced-boundary.md) has exactly two compatible generators and two distinct finite entropy values. The [theta counterexample](theta-global-audit.md) also proves that a locally isolated generator can coexist with a distant unbounded component. Thus local continuation and pairwise perturbations do not exhaust the full fiber. The [distinct-diagonal subclass](five-state-sparse-audit.md) and [cap-saturating path class](path-cap-uniqueness-audit.md) are classified, while a general five-state classification remains open.
...
The theta(100,100) target and its subsequent balanced-parameter boundary action are complete. [The sharp boundary proof](balanced-cone-bound.md) classifies the saved connected-hidden gap, and [D015](../evidence/RECORDS.md#d015) records the final entropy classification and resumption conditions. The most useful next scoped action is theorem-level novelty comparison and further independent review, not another completed-feasibility search. A hidden triangle still certifies unboundedness by the [hidden-pair theorem](hidden-pair-boundary-audit.md); failure of a numerical search never establishes a finite bound.
