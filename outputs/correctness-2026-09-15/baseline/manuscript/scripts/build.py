...
def main():
    ...
    tracked = ['main.tex', 'references.bib', 'main.pdf']
    tracked += sorted(p.relative_to(MANUSCRIPT).as_posix()
                      for p in (MANUSCRIPT/'supplementary').glob('*.tex'))
    tracked += sorted(p.relative_to(MANUSCRIPT).as_posix()
                      for p in (MANUSCRIPT/'figures').glob('*.pdf'))
