...
def main():
    ...
    jobs = [
        ...
        ('unit and independent numerical checks', ['-m','unittest','analysis.capabilities.test_tools',
          'analysis.revision.test_inference','analysis.revision.test_continuation','-v']),
    ]
    if args.build:
    ...
    destination = ROOT/'outputs/revision/reproduction.json'
    ...
    destination.write_text(json.dumps(report,indent=2)+'\n',encoding='utf-8')
    print('All requested checks passed; outputs/revision/reproduction.json',flush=True)
