...
- **Decision:** reopen scientific investigation at committed baseline `c7ff789` for the user's explicit request for an independent mathematical review, a substantial novelty plan and execution of the research loop. [Plan and falsification criteria](../outputs/novelty-loop-2026-09-19/PLAN.md).
- **Priority:** pursue an arbitrary-observation-graph characterization and finite-transform inverse before extending the special five-state numerical examples. Prior review has not resolved the one-uncovered-vertex gap. Preliminary primary-source work identifies Burgarth's all-time zero-forcing recovery as close prior art; therefore bare one-hidden-state identifiability cannot be assumed new.
- **Guardrails:** retain human-owned brief, accepted scripts/outputs and previous manuscript. New claims require explicit domains, exact all-time equality for exact fibers, proved divergent paths, adversarial boundary checks and primary-source comparison. Record substantive results before article changes or promotion. No novelty or publication guarantee follows from a successful calculation.

## E081

- **Observation / independent mathematical review:** [referee report](../outputs/novelty-loop-2026-09-19/REFEREE.md) audits baseline `c7ff789`, including the observation graph, inverse, local-ceiling, hidden-pair, splitting and selected global proof arguments. No concrete contradiction was found in that bounded review. The entire five-state cone proof/interval certificate was not independently recertified. The review identifies a substantive generalization and distinguishes exact image, local ceiling, rate calibration and the nonminimal equal-exit stratum.
- **Primary results / literature:** [source audit with exact locators and access limits](../outputs/novelty-loop-2026-09-19/LITERATURE.md), retrieved 19 September 2026. Daniel K. Burgarth, *Identifying combinatorially symmetric Hidden Markov Models*, ELA 34 (2018), 393-398, DOI 10.13001/1081-3810.3651; [full preprint](https://arxiv.org/pdf/1709.02932), Section 5 Eq. (18)/Theorem 2, provides observed-state zero-forcing recovery. Wagner and Timmer, *The Effects of Non-Identifiability on Testing for Detailed Balance in Aggregated Markov Models for Ion-Channel Gating*, Biophysical Journal 79 (2000), 2918-2924, [author-hosted original](https://jeti.uni-freiburg.de/papers/detailed_balance_printed.pdf), pp. 2919-2920, gives exact aggregate thermodynamic ambiguity. Vanluyten, Willems and De Moor, *Equivalence of state representations for hidden Markov models*, Systems & Control Letters 57 (2008), 410-419, [original](https://homes.esat.kuleuven.be/~sistawww/smc/jwillems/Articles/JournalArticles/2008.3.pdf), Proposition 2/Theorem 2, gives normalized-similarity equivalence under quasi-minimality. Ehrich, *Tightest bound on hidden entropy production from partially observed dynamics*, J. Stat. Mech. (2021), 083214, [preprint](https://arxiv.org/pdf/2105.08803), Section 4.1/Fig. 8 and discussion, supplies compatible discrete-time entropy ranges and apparently unbounded grid maxima. The coordinating reviewer also opened the underlying originals and checked the consequential theorem/criterion locators; no primary result was numerically rerun here.
- **Inference:** all-time one-hidden recovery is substantially anticipated through a resolved-kernel/restricted-semigroup bridge. Neither hidden thermodynamic ambiguity, normalized similarity nor the one-way entropy singularity is a new mechanism. The potential advance is the precise arbitrary-observation classification, finite bounded-measurement inverse and complete-source exact divergent construction with fixed N/trace/observed rates and no minimality. No matching theorem was found in the bounded close-source comparison. Priority remains unestablished, particularly against finite-frequency/Loewner realization, whose detailed original-source comparison remains incomplete.

## E082

- **Inference / conventionally proved:** [Theorem 1 and full denominator audit](../outputs/novelty-loop-2026-09-19/COVERAGE-THEOREM.md#2-finite-inverse-and-denominator-audit). On a fixed labeled finite irreducible reciprocal CTMC with a nonempty reverse-closed resolved-edge set, any three distinct positive full Laplace matrices recover the generator when exactly one vertex is uncovered. Any two suffice if an observed edge joins two neighbors of the hidden vertex, in particular at complete sources. If every vertex is covered, two suffice. Observed rates need not be known. Each source has a locally rational inverse, with locally Lipschitz dependence on the finite bounded measurements and row TV, and continuity under weak row-law convergence, without a common rate cap. This does not assume minimality or nonzero hidden incidence at every covered vertex.
- **Counterexample / conventionally proved sharpness:** for every N>=4, any prescribed two distinct positive Laplace arguments admit two one-hidden-vertex models with distinct entropy. [Exact four-state witness and arbitrary-sample proof](../outputs/novelty-loop-2026-09-19/COVERAGE-THEOREM.md#5-exact-witnesses-and-executed-independent-checks). At arguments 1 and 2, the full six-mark matrices coincide while entropies are log(2)/17 and 6log(2)/97, with positive gap 5log(2)/1649. They differ at argument 3. Added observed unit leaves prove the larger-N extension. This is finite-summary equality, not exact all-time data equality or matching stationary event intensity. The old at-most-three-state two-point entropy result remains valid.
- **Reproduced calculation / independent verification:** [independent proof](../outputs/novelty-loop-2026-09-19/ONE-HIDDEN-INDEPENDENT.md), [script](../analysis/novelty_loop/one_hidden_independent.py), [exact output](../outputs/novelty-loop-2026-09-19/one-hidden-independent.json): twelve sparse/complete reconstructions at N=4,5,6, full witness matrices, stationary currents and calibrated one-point witness. The [referee's selected-column inverse and final-draft checks](../outputs/novelty-loop-2026-09-19/referee_checks.py) provide a separate implementation; the [coordinator crosscheck](../analysis/novelty_loop/coverage_crosscheck.py) proves the generic two-sample interpolation and strict entropy variation symbolically. The shared model and SymPy engine are explicit. Finite checks corroborate the conventional universal argument; no formal proof or literature-wide sample-count priority is claimed.

## E083

- **Inference / conventionally proved:** [Theorems 2-3 and observation-design corollary](../outputs/novelty-loop-2026-09-19/COVERAGE-THEOREM.md). With the same fixed labeled model/observation class and unknown reciprocal support, a complete source has a singleton generator fiber iff at most one vertex is uncovered. With at least two uncovered vertices, its exact entropy image contains [sigma(Q0),infinity), preserving every observed rate and trace. The path stays complete before its excluded limit, has bounded rates and matches the all-time marked kernel exactly. Unequal external exit rows use the first-boundary normalized shear; equal rows use killed-generator lumping and an internal fixed-sum rate variation. These two cases include nonminimal sources.
- **Local criterion / conventions:** a finite local entropy ceiling exists iff the source is complete and at most one vertex is uncovered. This holds in maximum row TV and product weak topology and via finitely many bounded Laplace measurements. With observed rates and trace fixed, there is an additional zero-unobserved-rate-budget singleton exception. Every other sparse source permits an asymmetrically opened missing pair, with rate slack used to compensate trace if required. Minimum observation count at complete N-state sources is floor(N/2) reverse pairs, but their incidence must cover at least N-1 vertices. This is not a claim about unknown state caps, fixed sparse support, arbitrary extra tight caps, lumped states or blurred marks.
- **Counterexamples to provisional extensions:** hidden adjacency to all covered vertices does not ensure a ceiling when a covered-covered pair is missing. Conversely, observing every edge of a sparse connected source and fixing those rates and trace freezes the class, so completeness is not necessary in that exceptional calibrated class. These failed candidates are preserved in the proof and independent notes, rather than omitted from the final criterion.
- **Reproduced calculation / independent verification:** [referee checks](../outputs/novelty-loop-2026-09-19/referee_checks.json) verify equal-exit all-time lumping, an internal shear boundary reached before its external boundary and positive limiting reverse flux. [Coordinator exact crosscheck](../outputs/novelty-loop-2026-09-19/coverage-crosscheck.json) independently compares a symbolic resolvent with a rectangular lumping identity for the unit-rate K4 source, observes 0<->1, and obtains sigma=(1/4)log(1/m)+O(1), fixed trace -12, rates below 2 and exact entropy image [0,infinity). It enumerates every nonempty observed-edge subset through N=6; the matching/coverage proof supplies the all-N corollary. [Final-draft referee audit](../outputs/novelty-loop-2026-09-19/REFEREE.md) checks quantifiers, positivity, exceptional strata and law convergence. These are conventional proofs with exact support, not formal verification.

## D035

- **Decision:** promote the coverage/finite-measurement theorem package to conventionally proved research results after explicit construction, adversarial boundary checks and independent final-draft review. [E082](#e082), [E083](#e083). Do not promote it to a formally verified or priority-certified claim. Preserve the accepted manuscript as the preceding version; the new proof addendum and synthesis are the deliverables of this research cycle, with editorial integration a separately visible next action.
- **Reason for changed direction:** Burgarth narrows the novelty of bare all-time one-hidden recovery [E081](#e081). The broader complete-source exact dichotomy, sharp local criterion, finite-measurement sharpness and observation design consequence give the stronger target. Correct the two failed extensions: hidden-incidence-only completeness and fixed-trace criteria without the zero-budget exception. The old one-pair criterion is a special case, not refuted.
- **Next research priorities:** detailed finite-frequency primary comparison; conditioning-aware choice of coverage-sufficient observations and positive arguments; then exact entropy lower-range geometry. A finite-union-of-intervals/points route is recorded as a proposed next theorem, not accepted without a separate proof/audit. A broad numerical search or repeated historical simulations would not resolve current priority or sharp-conditioning uncertainty.

## D036

- **Decision:** finish the bounded scientific research loop with the independently reviewed coverage theorem package, exact witnesses, novelty comparison and ranked next cycle. [Current synthesis](../outputs/REPORT.md), [executed plan](../outputs/novelty-loop-2026-09-19/PLAN.md), [E081](#e081), [E082](#e082), [E083](#e083).
- **Executed checks:** all three independent/coordinator scientific scripts pass in the local pinned environment, including twelve sparse/complete reconstruction cases, arbitrary-two-argument entropy ambiguity, equal-exit all-time lumping, internal-first shear, and all observation sets through N=6. The final referee review has no outstanding mathematical finding. One optional wording change clarifies that the finite-test neighborhood radius, not the number of tests, is small; the verification receipt distinguishes that final wording from the audited document hash.
- **Stopping rationale and limits:** the scope has a substantive conventionally proved answer and explicit strongest countercases. Repeating finite examples or the old simulation campaign would not resolve the remaining finite-frequency priority, conditioning or exact lower-range questions. All bounded assignments are complete. The proof addendum is a new research result; the existing PDF/archive are preserved as the preceding version. Finish local artifact/provenance/map validation before handoff; no formal proof, priority certification, submission or recurring automation is claimed.

## D037

- **Decision / authorized scope:** the user requested pruning, commit and push. Keep the scientific investigation finished; prune the delivery, repair reproducibility defects and publish the checked repository changes to the existing `origin/main`.
- **Pruning:** shorten repeated synthesis/plan narrative, correct stale atlas counts and concurrent-work wording, and remove the duplicate `PRIOR-SYNTHESIS.md` whose relative links were invalid at its copied location. Its exact bytes remain recoverable as `c7ff789:outputs/REPORT.md`, linked from the current synthesis. Retain the full theorem, independent proofs, literature audit and accepted scientific payloads. Restore the parameter-geometry SVG after verifying that its only differences are generation time and internal IDs. The original research receipt is preserved as `research-verification.json`; the current [delivery verification](../outputs/novelty-loop-2026-09-19/verification.json) records these version boundaries.
- **Observed defect and repair:** an independent bounded pre-push review reproduced success under Python `-O` in all three new scientific entrypoints even though assertions were disabled. Add the standard early `__debug__` guard before imports/work. Extend the existing isolated entrypoint tests: all twelve entrypoints reject both `-O` and `PYTHONOPTIMIZE=1` without writing output. Normal reruns of the three new scripts pass with unchanged scientific payloads, excluding only declared provenance/timestamp/source-hash fields. No theorem statement or proof is changed.
- **Integrity:** preserve mixed-line-ending JSON receipts with a narrow `.gitattributes` rule, refresh reviewed atlas source hashes and verify the staged checkout before the authorized commit/push. This maintenance does not rerun the old statistical campaign, integrate the addendum into the PDF, or establish novelty/formal correctness.
