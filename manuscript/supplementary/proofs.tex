...
\end{equation}
Nonnegative input barycentric coordinates and output values impose two
conditions: the triangle contains both $(1,A)$ and $(-m,B_0)$, and it lies
in the wedge
\begin{gather}
...
at least $B_0$; its final saturated height is at most $B_0$. Writing
$H_*^{\rm sat}$ for Eq.~\eqref{eq:envelope} after $W=R_w(1-d)$, the chain is
\begin{equation}
 B_0\le H_c\le H_*\le H_*^{\rm sat}\le B_0.
 \label{eq:equality-chain}
\end{equation}
Thus every relaxation is equality. Strict increase under $W$ saturation
...
\begin{table}[htbp]
\caption{Endpoint equality conditions. Every row is necessary for a physical
triangle at $z=\zs$; existence is checked separately in
Appendix~\ref{app:certificate}.}\label{tab:equality-rigidity}
\begin{ruledtabular}
\begin{tabular}{p{0.24\textwidth}p{0.68\textwidth}}
Equality step & Reason and forced parameters\\
$H_*=H_*^{\rm sat}$ & Strict increase with $W$ at fixed $d$ forces
$W=R_w(1-d)$.\\
$H_*^{\rm sat}=B_0$ & The quadratic has its sole zero at
$t_*=-f_1/(2f_2)$; Eq.~\eqref{eq:quadratic} fixes $d$ and $W$.\\
$H_c=H_*$ & Strict left-coordinate increase forces
$(\ell,P)=(m+t_*,Lt_*)$; the unique right maximum forces
$(c,Q_v)=(c_*,Q_*^v)$.\\
$B_0=H_c$ & The upper input lies on the triangle's upper edge.\\
Physical normalization & Eq.~\eqref{eq:normalize} uniquely fixes the
column scaling; hidden permutation remains.\\
\end{tabular}
\end{ruledtabular}
\end{table}
...
An explicit complete-support example separates exact ambiguity from the
three-state instability under observation error. For $0\le e<2/3$, let
\begin{equation}
 Q_e=\begin{pmatrix}
 -6&1&2(1-e)&3+2e\\
 2&-9&3(1-e)&4+3e\\
 (4-6e)/(1-e)&5&-15-4e&(6-4e^2)/(1-e)\\
 6&5&4(1-e)&-15+4e
 \end{pmatrix}.\label{eq:complete-four}
\end{equation}
All twelve off-diagonals are positive, bounded, and row-normalized.
Observing only $x\leftrightarrow y$ in state order $(x,y,h,k)$ gives
$\Psi_{Q_e}=\Psi_{Q_0}$ for every such $e$, by the preceding shear or
by direct equality of the full resolvents. Only $q_{hx}$ tends to zero;
$q_{xh}\to2/3$ and $\pi_x=557/1420$. The reverse flux vanishes linearly
in $2/3-e$, while the forward flux tends to $557/2130>0$.
All other edge-flux ratios have positive finite limits. Consequently
\begin{equation}
 \sig(Q_e)=\frac{557}{2130}\log\frac1{2/3-e}+O(1).
\end{equation}
This is an exact compatible path within complete support, approaching an
inadmissible one-way boundary. Its full kernel identity and stationary
flux limit are independently checked from the displayed generator;
160-digit entropy evaluations are a further numerical check of the limit.
